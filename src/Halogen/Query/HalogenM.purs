...
data HalogenF state action slots output m a
  ...
  | Subscribe (SubscriptionId -> Emitter.Emitter action) (SubscriptionId -> a)
...
-- | finalization.
subscribe :: forall state action slots output m. Emitter.Emitter action -> HalogenM state action slots output m SubscriptionId
...
-- | is passed into an `Emitter`. This allows emitted queries to include the
-- | `SubscriptionId`, rather than storing it in the state of the component.
-- |
-- | When a component is disposed of any active subscriptions will automatically
-- | be stopped and no further subscriptions will be possible during
-- | finalization.
subscribe' :: forall state action slots output m. (SubscriptionId -> Emitter.Emitter action) -> HalogenM state action slots output m Unit
subscribe' esc = HalogenM $ liftF $ Subscribe esc (const unit)

-- | Unsubscribes a component from an `Emitter`. If the subscription associated
